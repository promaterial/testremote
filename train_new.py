...
# model = YOLO("yolo11m-seg.yaml").load("weights/yolo11m-seg.pt")
    model = YOLO(r'ultralytics/cfg/models/11/yolo11-addmoreDFF2.yaml')

    # 定义训练参数，添加默认值、范围和中文注释
    train_params = {
        'data': r"/content/drive/MyDrive/Steel_surface_defect_detection/ultralytics/cfg/datasets/mydata.yaml" ,
        #r"/content/drive/MyDrive/Code2/defect_data_colab.yaml",  # 数据集配置文件路径，需要自定义修改
        ...
        'project': "runs",  # 项目名称，保存训练结果的目录，默认值 None
        'name': "newdatav2-addDFF",  # 训练运行的名称，用于创建子目录保存结果，默认值 None
